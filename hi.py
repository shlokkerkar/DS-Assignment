...
df_database = pd.read_csv("./diabetes1.csv")

# Display the first few rows of the dab.head()

missing_values_in_database = df_database.isnull().sum()

# Display missing values for each column
missing_values_in_database

summary_stats_database = df_database.describe()

# Display summary statistics
summary_stats_database

df_database.hist(bins=10, figsize=(14, 12), layout=(3, 3), color='#3498db')
plt.tight_layout()
plt.show()

# Set up for correlation matrix
plt.figure(figsize=(12, 8))
corr_matrix_diabetes = df_database.corr()
...
for col in cols_zeros:
    df_database[col] = df_database[col].replace(0, df_database[col].median())
...
normalize = ['Pregnancies', 'Glucose', 'BloodPressure', 'SkinThickness', 'Insulin', 'BMI', 'DiabetesPedigreeFunction', 'Age']

# Initialize the MinMaxScaler
scaler = MinMaxScaler()

# Normalize the selected features
df_database[normalize] = scaler.fit_transform(df_database[normalize])

# Display the first few rows after normalization
df_database.head()

correlation_matrix = df_database.corr().abs()
...
X = df_database.drop('Outcome', axis=1)  # Features
y = df_database['Outcome']               # Target
...
mutual_selector = SelectKBest(score_func=mutual_info_classif, k='all')
mutual_selector.fit(X, y)

# Get feature scores for both methods
chi2_scores = chi2_selector.scores_
mutual_info_scores = mutual_selector.scores_
